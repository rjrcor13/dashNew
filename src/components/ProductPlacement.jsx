...
const ProductPlacement = () => {
	...
	const [product, setProduct] = useState(''); // State for selected product
	const [quantity, setQuantity] = useState('');

	const [editIndex, setEditIndex] = useState(-1); // Index of the row being edited (-1 means not editing)
	...
	const handleEdit = (index) => {
		...
		setEditIndex(index);
		setEditCustomer(editedRow.customer);
		setEditProduct(editedRow.product);
		setEditQuantity(editedRow.quantity.toString());
	};

	const handleSaveEdit = () => {
		// Update the edited row in tableData
	...
	return (
		<div>
			<Header
				...
				subtitle="Monitor your product placement."
			/>
			<div className="flex  flex-wrap ">
				<div className="w-full sm:w-1 md:w-1/2 lg:w-1/2 xl:w-1/2 ">
					<form name="placementForm" onSubmit={handleUpload}>
						...
						<div className="form-control w-full max-w-xl my-5 ">
							...
							<input
								...
								className="input input-bordered w-full md:w-3/4 lg:w-3/4 xl:w-3/4 "
								id="quantity"
								name="quantity"
								value={quantity}
			...
			{showToast && (
				...
				// 	type="error" // Replace with the appropriate type
				// />
